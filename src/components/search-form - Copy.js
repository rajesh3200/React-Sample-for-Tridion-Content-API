...
export default class SearchForm extends React.Component {
   ...
   //templateId on aws dev=3844
    this.state = {
	  ...
	  lastNode:"",
	  categoryId:1178,
	  keywordId:14298, //15269
	  defaultKeywordId:14298
    };    
  ...
  _sortOrderDropdownChangeHandler(sortOrder){   
	...
	_fetchTaxonomies() {
		var query = `query sitemapSubtree($namespaceId: Int!, $publicationId: Int!, $taxonomyNodeId: String, $includeAncestors: Boolean) {
			  sitemapSubtree(namespaceId: $namespaceId, publicationId: $publicationId, taxonomyNodeId: $taxonomyNodeId, includeAncestors: $includeAncestors) {
				...TaxonomyItemFields
				...RecurseItems
			  }
			}
			...
			data: JSON.stringify({ 
				query:	query, 
						variables:{
							...
							"taxonomyNodeId": "t1178", //"t"+this.state.categoryId.toString(),
							"includeAncestors": true
						}
					}),
			/*url: 'taxonomies.json',*/
			/*url: `${this.state.siteUrl}api/BrokerQuery/classifications`,*/
			//url: `http://holdings.sdldemo.com/api/BrokerQuery/classifications`,
			//url: `${location.origin}:8083/cd/api`,
			url:`http://10.77.3.138:8081/cd/api`,
	...
	_fetchPressReleases(page = 0, searchKeyword = "") {      
		...
		//} 
    	// Grpahql Query
		var query = `query componentPresentations($filter: InputComponentPresentationFilter!, $sort: InputSortParam, $first: Int, $after: String, $namespaceId: Int!, $publicationId: Int!) {
						componentPresentations(filter:$filter, sort:$sort, first:$first, after:$after, namespaceId: $namespaceId, publicationId: $publicationId){
							edges {
								cursor
									node {
										itemId
